...
baseFolderName = 'P150_S1_R20.0_W1.0_MP01.4_SS03.0_DP2.0_r';
for i=1:20
  ...
  if any(ismember(T_Pax.passenger_ID,5))
    ...
    for ff = 1:this_fleetSize % one for each bus. Could do multiple loops
      ...
      if any(ismember(this_bus_cus ,T_Pax.cus(T_Pax.passenger_ID==5)))
      ...
      th = text(T_Pax.passenger_X(T_Pax.passenger_ID==5),T_Pax.passenger_Y(T_Pax.passenger_ID==5), string(T_Pax.passenger_ID(T_Pax.passenger_ID==5)), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right');
      sh =  scatter(T_Pax.passenger_X(T_Pax.passenger_ID==5),T_Pax.passenger_Y(T_Pax.passenger_ID==5),'or');
      sh.SizeData = 50;
    end
